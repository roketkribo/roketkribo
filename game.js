...
function drawEarth() {
    ...
    ctx.beginPath();
    ctx.ellipse(x - r * 0.25, y - r * 0.05, r * 0.4, r * 0.22, 0.3, 0, Math.PI * 2);
    ctx.fill();

    ctx.beginPath();
    ctx.ellipse(x + r * 0.1, y + r * 0.08, r * 0.3, r * 0.18, -0.4, 0, Math.PI * 2);
    ctx.fill();

    // awan
    ctx.fillStyle = "rgba(255,255,255,0.85)";
    ctx.beginPath();
    ctx.ellipse(x - r * 0.1, y - r * 0.35, r * 0.45, r * 0.12, 0.1, 0, Math.PI * 2);
    ctx.fill();

    ctx.beginPath();
    ctx.ellipse(x + r * 0.05, y - r * 0.15, r * 0.35, r * 0.09, -0.2, 0, Math.PI * 2);
    ctx.fill();
}
...
const LEADERBOARD_KEY = "roketKriboLeaderboard";
...
function updateLeaderboard(nick, newScore) {
    let list = loadLeaderboard();
    list.push({ nick, score: newScore, time: Date.now() });
...
// Roket horizontal mirip gambar referensi
function drawRocket() {
    ctx.save();
    ...
    ctx.quadraticCurveTo(x + bodyW, y + bodyH, x + bodyW - radius, y + bodyH);
    ...
    const gradFlame = ctx.createLinearGradient(
        flameBaseX - flameLen, flameCenterY,
        flameBaseX, flameCenterY
    );
...
function createMeteorColumn(gapCount, gapHeight) {
    ...
    while (gaps.length < gapCount) {
        let g = usableTop + Math.random() * (usableBottom - usableTop);
...
function gameOver() {
    ...
    updateLeaderboard(currentNickname, score);
...
function gameLoop() {
    ...
    if (gameState === "playing") {
        ...
        // menu & gameover: roket diam sebagai dekorasi
        updateRocketColor();
        updateFlameLength();
        drawRocket();
    }
...
// mulai pertama kali (di menu, tanpa musik)
initRocket();
resetGame();
gameLoop();
